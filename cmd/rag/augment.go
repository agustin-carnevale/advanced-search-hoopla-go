...
func newAugmentCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "augment <query>",
		Short: "Use RAG to augment the results with an LLM.",
		Run: func(cmd *cobra.Command, args []string) {
			...
			llmAnswer, err := llms.ResultsAugmentation(ctx, query, resultsStr)
			...
			fmt.Println("LLM Augmented Answer:")
...
func init() {
	augmentCmd := newAugmentCmd()
	RAGCmd.AddCommand(augmentCmd)
}
